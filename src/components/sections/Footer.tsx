...
const socialLinks = [
  { icon: Instagram, href: 'https://instagram.com/worldcaseuy', label: 'Instagram' },
  { icon: Facebook, href: 'https://facebook.com/worldcaseuy', label: 'Facebook' },
];
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map((social) => (
              ...
              >
                <social.icon 
                  className="w-5 h-5 text-muted-foreground group-hover:text-primary transition-all duration-300 group-hover:rotate-[360deg]" 
                  aria-hidden="true"
                />
              </a>
            ))}
          ...
          <Button
            ...
            className={`relative overflow-hidden transition-all duration-500 ${
              isElevating ? 'bg-primary text-primary-foreground' : ''
            }`}
            aria-label="Volver arriba"
          >
            <ArrowUp 
              className={`w-5 h-5 transition-transform duration-500 ${
                isElevating ? '-translate-y-10' : ''
              }`}
              aria-hidden="true"
            />
            <ArrowUp 
              className={`w-5 h-5 absolute transition-transform duration-500 ${
                isElevating ? 'translate-y-0' : 'translate-y-10'
              }`}
              aria-hidden="true"
            />
          </Button>
